...
dnl ##
dnl ##  configure.in:
dnl ##    AC_CHECK_HP_TIMING
dnl ##

AC_DEFUN(AC_CHECK_HP_TIMING,[dnl
AC_MSG_CHECKING(for HP timing option)
AC_ARG_ENABLE(hp_timing,dnl
[  --enable-hp_timing        build using HP timing  (default=no)],
[dnl
CFLAGS="$CFLAGS -DENABLE_HP_TIMING"
msg="enabled"
],[
msg="disabled"
])dnl
...
dnl ##
dnl ##  Libthread_db debug Support
dnl ##
dnl ##  configure.in:
dnl ##    AC_CHECK_THREAD_DB
dnl ##

AC_DEFUN(AC_CHECK_THREAD_DB,[dnl
AC_MSG_CHECKING(for compilation inclusion of libthread_db)
AC_ARG_ENABLE(thread_db,dnl
[  --enable-thread_db        build for libthread_db (default=no)],
[dnl
CFLAGS="$CFLAGS -DTHREAD_DB"
msg="enabled"
],[
msg="disabled"
])dnl
