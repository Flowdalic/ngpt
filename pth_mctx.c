...
#ifdef __ia64__
#undef uc_sigmask
#define uc_sigmask uc_mcontext.sc_mask
#endif
#endif
...
#ifdef PTH_DEBUG
#define  _pth_mctx_switch_debug pth_debug1(SWITCH_DEBUG_LINE);
#else
...
intern int pth_sig_switch = __SIGRTMIN + 1;
intern int pth_sig_shared = __SIGRTMIN + 2;
intern pth_qlock_t pth_usr1_lock = pth_qlock_st_INIT_UNLOCKED;
    ...
    pth_mctx_t *mctx, void (*func)(void), char *sk_addr_lo, char *sk_addr_hi)
{
    ...
    sigemptyset(&(mctx->uc.uc_sigmask));
...
static void pth_mctx_set_bootstrap(pth_descr_t);
...
#error "unknown mctx stack setup"
    ...
    _pth_acquire_lock(&pth_usr1_lock, descr->tid);

    /*
     * Preserve the pth_sig_switch signal state, block pth_sig_switch,
     * and establish our signal handler. The signal will
     * later transfer control onto the signal stack.
     */
    sigemptyset(&sigs);
    sigaddset(&sigs, pth_sig_switch);
    pth_sc(sigprocmask)(SIG_BLOCK, &sigs, &osigs);
    sa.sa_handler = pth_mctx_set_trampoline;
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = SA_ONSTACK;
    if (__libc_sigaction(pth_sig_switch, &sa, &osa) != 0) {
...
#error "unknown mctx stack setup"
     ...
     * called.
     */
    descr->mctx_called = FALSE;
    k_tkill(descr->tid, pth_sig_switch);
    pth_debug1("pth_mctx_set: back from kill");
    sigfillset(&sigs);
    sigdelset(&sigs, pth_sig_switch);
    while (!descr->mctx_called)
        sigsuspend(&sigs);
    pth_debug1("pth_mctx_set: back from suspend");
    ...
    /*
     * Restore the old pth_sig_switch signal handler and mask
     */
    __libc_sigaction(pth_sig_switch, &osa, NULL);
     ...
     */
    descr->mctx_creating      = mctx;
    descr->mctx_creating_func = func;
    memcpy((void *)&descr->mctx_creating_sigs, &osigs, sizeof(sigset_t));
     ...
     */
    if (pth_mctx_save((pth_mctx_t *)&descr->mctx_caller) == 0)
        longjmp(*((jmp_buf *)descr->mctx_trampoline), 1);
...
static void pth_mctx_set_trampoline(int sig)
{
     ...
     * platforms).
     */
    if (setjmp(*((jmp_buf *)descr->mctx_trampoline)) == 0) {
	pth_debug1("pth_mctx_set_trampoline: return from caller");
        descr->mctx_called = TRUE;
        return;
    }
    pth_debug1("pth_mctx_set_trampoline: re-entered from caller");
     ...
     */
    pth_mctx_set_bootstrap(descr);
}

/* boot function */
static void pth_mctx_set_bootstrap(pth_descr_t descr)
{
     ...
     */
    pth_sc(sigprocmask)(SIG_SETMASK, (sigset_t *)&descr->mctx_creating_sigs, NULL);
     ...
     */
    mctx_starting      = descr->mctx_creating;
    mctx_starting_func = descr->mctx_creating_func;
    ...
    pth_mctx_switch((pth_mctx_t *)mctx_starting, (pth_mctx_t *)&descr->mctx_caller);
    ...
    mctx_starting_func();
    pth_debug1("pth_mctx_set_trampoline_jumpin: aborting");
    abort();
}
