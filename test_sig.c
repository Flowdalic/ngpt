...
static void *inthandler(void *_arg)
{
    ...
    for (n = 0; n < 3; n++) {
	 ...
	 __pth_dumpstate(stderr);
    }

    fprintf(stderr, "inthandler: cancelling child1, child2, child3 and child4\n");
    pth_cancel(child1);
    pth_cancel(child2);
    pth_cancel(child3);
    pth_cancel(child4);
...
int main(int argc, char *argv[])
{
    ...
    child3 = pth_spawn(attr, child, (void *)"child3");
    pth_attr_set(attr, PTH_ATTR_NAME, "child4");
    child4 = pth_spawn(attr, child, (void *)"child4");
