...
#ifdef ASSERTS_ON
#define pth_assert_owned(q) \
    pth_assert((q)->q_lock.lock > 0 && (q)->q_lock.owner == (int)current_tid())
#else
#define pth_assert_owned(q)
#endif
...
intern void pth_pqueue_insert(pth_pqueue_t *q, int prio, pth_t t)
{
    ...
    if (q->q_head == NULL || q->q_num == 0) {
        ...
        c = q->q_head;
        p = c->q_prio;
	pth_assert(((c != NULL) && (c->q_next != NULL) && (c->q_prev != NULL)));
