...
static void parse_args( int argc, char *argv[] )
{
	...
	while ( (opt = getopt( argc, argv, "b:d:t:Ds?" )) != EOF ) {
	    switch ( opt ) {
		case 's':
		    use_shared = 1;
		    break;
	...
	if ( errflag ) {
		fprintf( stderr, "usage: %s [-b <num>] [-d <num>] [-t <num>] [-s] [-D]", argv[0] );
		...
		fprintf( stderr, "\t-s      \twrap with shared mutex\n" );
...
{
	int		rc, ind, total, i_initialized = 0;
	pthread_mutexattr_t myattr;
	pthread_t	root_thread;
	shared_area_t	*sa = NULL;
	...
	if (use_shared == 1) {
	    ...
	    if (shared_fd == 0) {
		shared_fd = open("test_str03m", O_RDWR, (S_IRWXU | S_IRWXG | S_IRWXO));
		if (shared_fd < 0) {
		    fprintf(stderr, "unable to open shared memory\n");
		    exit(1);
		}
		sa = (shared_area_t *)mmap(NULL, (sizeof(struct shared_area)), 
	    ...
	    if (sa->initialized != 1) {
		fprintf(stderr,"shared memory was not initialized.\n");
		pthread_mutexattr_init(&myattr);
		pthread_mutexattr_setpshared(&myattr, PTHREAD_PROCESS_SHARED);
		if ((rc = pthread_mutex_init(&(sa->mutex), &myattr))) {
		    fprintf( stderr, "pthread_mutex_init(test_mutex): %s\n",
			     strerror(rc) );
		    exit( 7 );
		}
		fprintf(stderr,"mutex was initialized.\n");
		sa->initialized = 1;
		i_initialized = 1;
		msync(sa, sizeof(struct shared_area), MS_INVALIDATE | MS_SYNC);
	    } else
		fprintf(stderr,"shared memory was initialized.\n");

	    if (i_initialized == 1) {
		fprintf(stderr,"I did initialize, spinning...\n");
		fprintf(stderr,"waiting for partner to lock.\n");
		while (sa->locked != 1) {
		    sleep(1);
		    continue;
		}
		fprintf(stderr,"partner is locked.\n");
	    }

	    if ((rc = pthread_mutex_lock(&(sa->mutex)))) {
		fprintf(stderr, "unable to grab test_mutex, rc = %d\n", rc);
		exit( 7);
	    }
	    fprintf(stderr,"I am locked.\n");
	    sa->locked = 1;
	    msync(sa, sizeof(struct shared_area), MS_INVALIDATE | MS_SYNC);
