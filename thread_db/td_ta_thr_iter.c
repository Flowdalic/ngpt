	      ...
	      size_t cnt, pthread_descr descr)
{
  ...
  err = ps_pdread (ta->ph, descr, &pds, sizeof_descr);
  if (err != PS_OK) {
    lib_td_debug(__FUNCTION__ ": failed reading thread descriptor");
    /* This return looks goofy, but ngpt might have deallocated the 
     * thread tcb if it terminated. We say with this that if we can't
     * read the tcb we assume the thread is gone and skip the call to
     * the iterator. This stops a ugly failure condition where iter 
     * aborts when a thread's structures are deallocated.
     */
    return TD_OK;	/* skip over this dead puppy if read failed  */
  }
  ...
  if (__thread_prio_get (&pds) < ti_pri)
    return TD_OK;
...
# define num 1
  ...
  if (! ta_ok (ta)) {
    lib_td_debug(__FUNCTION__ ": bad ta argument");
    return TD_BADTA;
  }
  ...
  result = handle_descr (ta, callback, cbdata_p, state, ti_pri, 0,
			 phc[0].h_descr);
  if (result != TD_OK) {
    lib_td_debug(__FUNCTION__ ": failed main descriptor check");
    return result;
  }
  result = handle_descr (ta, callback, cbdata_p, state, ti_pri, 1,
			 phc[1].h_descr);
  if (result != TD_OK) {
    lib_td_debug(__FUNCTION__ ": failed manager descriptor check");
    return result;
  }
  ...
  if (ps_pdread (ta->ph, ta->handles + 2, &phc[2],
    ...
    lib_td_debug(__FUNCTION__ ": failed reading all descriptors");
    return TD_ERR;	/* XXX Other error value?  */
  }
  ...
  for (cnt = 2; cnt < pthread_threads_max && num > 0; ++cnt)
	...
	result = handle_descr (ta, callback, cbdata_p, state, ti_pri, cnt,
			       phc[cnt].h_descr);
	if (result != TD_OK) {
	  lib_td_debug(__FUNCTION__ ": failed handling descriptors");
	  break;
	}
      }
