...
/* initialize ring; O(1) */
#if cpp
static inline void pth_ring_init(pth_ring_t *r)
{
    r->r_hook  = NULL;
    r->r_nodes = 0;
    return;
...
#if cpp
static inline void pth_ring_delete(pth_ring_t *r, pth_ringnode_t *rn)
{
...
#if cpp
static inline void pth_ring_prepend(pth_ring_t *r, pth_ringnode_t *rn)
{
    r->r_hook = rn;
    if (r->r_hook == NULL) {
        rn->rn_next = rn;
...
#if cpp
static inline void pth_ring_append(pth_ring_t *r, pth_ringnode_t *rn)
{
    if (r->r_hook == NULL) {
        r->r_hook = rn;
        rn->rn_next = rn;
