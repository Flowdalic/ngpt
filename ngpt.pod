...
B<ngpt> - Next Generation POSIX Threading
...
B<ngpt> is a very portable POSIX/ANSI-C based library for Unix platforms which
...
B<ngpt> also provides an optional emulation API for POSIX.1c threads
(`Pthreads') which can be used for backward compatibility to existing
multithreaded applications. See B<ngpt>'s pthread(3) manual page for
details.
...
of threading, to understand B<ngpt>, you need only basic knowledge about
...
=item B<o> B<preemtive> vs. B<non-preemtive> thread scheduling
...
their bad aspects? That's the goal of B<ngpt>. B<ngpt> implements
...
working with B<ngpt>. The following list summarizes a few essential
points:
...
occurs, as it is the case for preemptive scheduling. That's why B<ngpt>
...
possibility. Especially this means that under B<ngpt> more existing
...
This means that B<ngpt> runs on almost all Unix kernels, because the
kernel does not need to be aware of the B<ngpt> threads (because they
...
To understand the B<ngpt> Application Programming Interface (API), it
helps to first understand the life cycle of a thread in the B<ngpt>
...
The purpose of the B<NEW> queue has to do with the fact that in B<ngpt>
...
In the following the B<ngpt> I<Application Programming Interface> (API)
...
Unix tradition, B<ngpt> functions use special return values (C<NULL>
...
This initializes the B<ngpt> library. It has to be the first B<ngpt> API
...
This kills the B<ngpt> library. It should be the last B<ngpt> API function call
...
usual way to terminate a B<ngpt> application is either a simple
...
This is a generalized query/control function for the B<ngpt> library.  The
...
of the internal B<ngpt> library state is written to. The main information
...
current B<ngpt> library version. I<V> is the version, I<RR> the revisions,
I<LL> the level and I<T> the type of the level (alphalevel=0, betalevel=1,
patchlevel=2, etc). For instance B<ngpt> version 1.0b1 is encoded as 0x100101.
...
Attribute objects are used in B<ngpt> for two things: First stand-alone/unbound
...
=item C<PTH_ATTR_CREATE_SUSPENDED> (read write) [C<int>]

Whether the thread was created suspended (C<TRUE>) or not (C<FALSE>).  This
attribute can be specified prior to calling C<pth_spawn> and queried thereafter.
Changing it after it is bound to a thread is meaningless.
...
There is no B<ngpt>-internal limit on the number of threads one can spawn,
except the limit implied by the available virtual memory. B<ngpt> internally
...
cooperating part of B<ngpt>.  A thread I<has not> to yield execution, of
...
of the non-preemptive nature of B<ngpt> it can last longer (when another thread
...
`C<pth_exit(PTH_CANCELED)>' at one of his cancellation points.  In B<ngpt>
...
C<PTH_FDMODE_ERROR> on error. Keep in mind that since B<ngpt> 1.1 there is no
longer a requirement to manually switch a file descriptor into non-blocking
mode in order to use it. This is automatically done temporarily inside B<ngpt>.
...
=item int B<pth_getstacksize>(pth_t I<thr>, int * I<stacksize>);

This function returns stacksize for the given C<pth_t> I<thr> in the integer
pointer, I<stacksize>.

=item void * B<pth_getcontext>(pth_t I<thr>);

This function returns the current context for the given C<pth_t> I<thr>. 

=item int * B<pth_geterrno>(void);

This function returns the current value of errno.

=item Sfdisc_t *B<pth_sfiodisc>(void);

This functions is always available, but only reasonably usable when B<ngpt>
...
comprehensive B<Sfio> I/O library with the B<ngpt> threading library. Then this
...
B<ngpt> supports POSIX style thread cancellation via pth_cancel(3) and the
...
B<ngpt> has a very flexible event facility which is linked into the scheduler
...
the second additional argument. Keep in mind that the B<ngpt> scheduler doesn't
...
C<FALSE>. Once it returned C<TRUE> the thread will be awakend. The
...
system like B<ngpt> this might sound unnecessary at the first look, because a
...
because of the non-preemptive scheduling nature of B<ngpt>, it can be awakened
...
non-preemptive scheduling nature of B<ngpt>, it can be awakened later, of
...
This is the B<ngpt> thread-related equivalent of POSIX sigprocmask(2) respectively
pthread_sigmask(3). The arguments I<how>, I<set> and I<oset> directly relate
to sigprocmask(2), because B<ngpt> internally just uses sigprocmask(2) here. So
...
is I<only> intended to show you the look and feel of B<ngpt>.
...
environment for a B<ngpt> based program. The possibilities supported by B<ngpt>
...
just adding the following C<Makefile>:
 ...
 $ vi Makefile
 | CC      = cc
 | CFLAGS  = `ngpt-config --cflags`
 | LDFLAGS = `ngpt-config --ldflags`
 | LIBS    = `ngpt-config --libs`
 |
 | all: foo
 | foo: foo.o
...
B<ngpt> installation via its C<ngpt-config> program. This approach is
...
also be able to build against an uninstalled B<ngpt>, that is, against
a B<ngpt> source tree which was just configured and built, but not
installed. Third, it would be also useful to allow checking of the
B<ngpt> version to make sure it is at least a minimum required version.
And finally, it would be also great to make sure B<ngpt> works correctly
...
provided by B<ngpt>. For this, we establish the following three files:
...
file containing B<ngpt>'s C<AC_CHECK_PTH> macro. Then we generate the final
C<configure> script out of this C<aclocal.m4> file and the C<configure.in>
file:

 $ aclocal --acdir=`ngpt-config --acdir`
 $ autoconf
...
immediately can be used to build C<foo> (assuming that B<ngpt> is already
installed somewhere, so that C<ngpt-config> is in C<$PATH>):
 ...
 gcc -L/usr/local/lib -o foo foo.o -lngpt

If B<ngpt> is installed in non-standard locations or C<ngpt-config>
...
the C<--prefix> option when B<ngpt> was installed).
...
end-user first has to install B<ngpt> just to allow our C<foo> package to
compile. For this, it is a convinient practice to include the required
libraries (here B<ngpt>) into the source tree of the package (here C<foo>).
B<ngpt> ships with all necessary support to allow us to easily achieve this
approach. Say, we want B<ngpt> in a subdirectory named C<pth/> and this
...
second argument to C<AC_CHECK_PTH> which indicates that B<ngpt> can be found in
the subdirectory named C<pth/>. Additionally we specified that the
C<--disable-tests> option of B<ngpt> should be passed to the C<pth/>
subdirectory, because we need only to build the B<ngpt> library itself. And we
...
C<aclocal.m4> file with the contents of B<ngpt>'s C<AC_CHECK_PTH> macro.
...
B<ngpt> distribution to the C<foo> source tree and just rename it to C<pth/>:

 $ gunzip <pth-X.Y.Z.tar.gz | tar xvf -
 $ mv pth-X.Y.Z pth

Optionally to reduce the size of the C<pth/> subdirectory, we can strip down
the B<ngpt> sources to a minimum with the I<striptease> feature:
...
Now when we configure and build the C<foo> package it looks similar to this:
 ...
 gcc -Lngpt -o foo foo.o -lngpt

As you can see, B<autoconf> now automatically configures the local
(stripped down) copy of B<ngpt> in the subdirectory C<pth/> and the
C<Makefile> automatically builds the subdirectory, too.

=head1 SYSTEM CALL WRAPPER FACILITY

B<ngpt> per default uses an explicit API, including the system calls. For
...
magic B<ngpt> cannot provide per default because it's not really portable.
Nevertheless B<ngpt> provides a two step approach to solve this problem:

=head2 Soft System Call Mapping

This variant is available on all platforms and can I<always> be enabled by
building B<ngpt> with C<--enable-syscall-soft>. This then triggers some
...
function exists and there it can be enabled by building B<ngpt> with
C<--enable-syscall-hard>. This then builds wrapper functions (for instances
read(3)) into the B<ngpt> library which internally call the real B<ngpt>
...
accept(2), read(2), write(2), recv(2), send(2), recvfrom(2), sendto(2).
...
B<ngpt> is very portable because it has only one part which perhaps has
...
C<pth_mctx.c> for details]. Any other B<ngpt> code is POSIX and ANSI C
based only.
...
B<ngpt> dispatcher switches also the global Unix C<errno> variable [see
...
The B<ngpt> event manager is mainly select(2) and gettimeofday(2) based,
...
B<ngpt> (intentionally) provides no replacements for non-thread-safe
...
The B<ngpt> library was designed and implemented between February and
...
B<ngpt> was then implemented in order to combine the I<non-preemptive>
...
If you think you have found a bug in B<ngpt>, you should send a report as
...
I<bluethread-users@ibm.com> mailing list by sending an Email to
I<bluethread-users-request@ibm.com> with `C<subscribe bluethread-users>' (or
`C<subscribe bluethread-users> I<address>' if you want to subscribe
from a particular Email I<address>) in the body. Then you can
discuss your issues with other B<ngpt> users by sending messages to
I<bluethread-users@ibm.com>. Currently (as of January 2000) you can reach about
