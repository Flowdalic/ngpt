dnl ##
dnl ##  NGPT - Next Generation Threading
...
dnl ##  USA.
dnl ##
dnl ##  ngpt.m4: Autoconf macro for locating ngpt from within
...
if test ".$with_pth" != .no; then
    ...
    if test ".$with_pth" = .yes; then
        ...
        changequote(, )dnl
        _pth_version=`(ngpt-config --version) 2>/dev/null |\
                      sed -e 's/^.*\([0-9]\.[0-9]*[ab.][0-9]*\).*$/\1/'`
        changequote([, ])dnl
        if test ".$_pth_version" != .; then
            _pth_location=`ngpt-config --prefix`
            _pth_type='installed'
            _pth_cppflags=`ngpt-config --cflags`
            _pth_cflags=`ngpt-config --cflags`
            _pth_ldflags=`ngpt-config --ldflags`
            _pth_libs=`ngpt-config --libs`
        fi
        ...
        if test ".$_pth_found" = .no; then
            for _dir in $with_pth/bin $with_pth; do
                if test -f "$_dir/ngpt-config"; then
                    test -f "$_dir/ngpt-config.in" && continue # ngpt-config in source tree!
                    changequote(, )dnl
                    _pth_version=`($_dir/ngpt-config --version) 2>/dev/null |\
                                  sed -e 's/^.*\([0-9]\.[0-9]*[ab.][0-9]*\).*$/\1/'`
                    changequote([, ])dnl
                    if test ".$_pth_version" != .; then
                        _pth_location=`$_dir/ngpt-config --prefix`
                        _pth_type="installed"
                        _pth_cppflags=`$_dir/ngpt-config --cflags`
                        _pth_cflags=`$_dir/ngpt-config --cflags`
                        _pth_ldflags=`$_dir/ngpt-config --ldflags`
                        _pth_libs=`$_dir/ngpt-config --libs`
                        _pth_found=yes
                        break
                    fi
                fi
        ...
        if test ".$_pth_found" = .no; then
            ...
            for _file in x `find $with_pth -name "libngpt.[aso]" -type f -print`; do
                test .$_file = .x && continue
                _dir=`echo $_file | sed -e 's;[^/]*$;;' -e 's;\(.\)/$;\1;'`
                _pth_ldflags="-L$_dir"
                _pth_libs="-lngpt"
    ...
    if test ".$with_pth_test" = .yes; then
        ...
        if test ".$_ok" != .yes; then
            ...
            the GNU Pth library libngpt.a was not found.
            We used the following build environment:
            >> CC="$CC"
            >> CFLAGS="$CFLAGS"
            >> LDFLAGS="$LDFLAGS"
        ...
        if test ".$_ok" != .yes; then
            if test ".$_ok" = .no; then
                ...
                means that the GNU Pth shared library libngpt.so is present
